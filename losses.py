...
def d_bce_loss(mask):
    def loss(y_true, y_pred):
        d_bce_loss = BinaryCrossentropy()(y_true, y_pred)
        return d_bce_loss
...
def trajLoss(real_traj, gen_traj):
    def loss(y_true, y_pred):
        ...
        ce_category = loss_fn(real_traj[1], gen_traj[1])
        ce_day = loss_fn(real_traj[2], gen_traj[2])
        ce_hour = loss_fn(real_traj[3], gen_traj[3])
