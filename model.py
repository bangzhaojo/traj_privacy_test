...
class LSTM_TrajGAN():
    ...
    def train(self, epochs=200, batch_size=256, sample_interval=10):
        ...
        # Prepare tf.data.Dataset for efficient batch retrieval
        dataset = tf.data.Dataset.from_tensor_slices(tuple(X_train)).batch(batch_size)
        ...
        for epoch in range(1, epochs + 1):
            for step, real_trajs_bc in enumerate(dataset):
                batch_size = tf.shape(real_trajs_bc[0])[0]
                noise = tf.random.normal((batch_size, self.latent_dim))

                real_bc = tf.ones((batch_size, 1))
                syn_bc = tf.zeros((batch_size, 1))

                real_trajs_bc = list(real_trajs_bc)
                real_trajs_bc.append(noise)  # noise

                with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
                    # Generate synthetic trajectories
                    gen_trajs_bc = self.generator(real_trajs_bc, training=True)

                    # Discriminator losses
                    real_output = self.discriminator(real_trajs_bc[:4], training=True)
                    fake_output = self.discriminator(gen_trajs_bc[:4], training=True)

                    d_loss_real = tf.keras.losses.BinaryCrossentropy(from_logits=True)(real_bc, real_output)
                    d_loss_syn = tf.keras.losses.BinaryCrossentropy(from_logits=True)(syn_bc, fake_output)
                    d_loss = (d_loss_real + d_loss_syn) * 0.5

                    # Generator loss
                    g_loss = tf.keras.losses.BinaryCrossentropy(from_logits=True)(real_bc, fake_output)

                # Get and apply gradients for discriminator
                gradients_of_discriminator = disc_tape.gradient(d_loss, self.discriminator.trainable_variables)
                discriminator_optimizer.apply_gradients(
                    zip(gradients_of_discriminator, self.discriminator.trainable_variables))

                # Get and apply gradients for generator
                gradients_of_generator = gen_tape.gradient(g_loss, self.generator.trainable_variables)
                generator_optimizer.apply_gradients(zip(gradients_of_generator, self.generator.trainable_variables))

                if step % 100 == 0:
                    print("[%d/%d][%d] D loss: %.4f | G loss: %.4f" % (epoch, epochs, step, d_loss, g_loss))
    ...
    def train_v1(self, epochs=200, batch_size=256, sample_interval=10):
        ...
        for epoch in range(1,epochs+1):
            ...
            #print('real_trajs_bc: ', type(real_trajs_bc))
            #print('real_bc: ', type(real_bc))
